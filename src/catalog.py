...
async def sync_catalog(path: str | Path) -> SyncReport:
    ...
    categories = {category.name.casefold(): category for category in await Category.get_all()}
    products = {product.sku.casefold(): product for product in await Product.get_all()}
    source_skus = {row.sku.casefold() for row in rows}

    products_created = products_updated = products_hidden = categories_created = 0
    for row in rows:
